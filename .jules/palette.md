...
## 2026-09-15 - Accessibility on interactive headers
**Learning:** Replaced a bare `GestureDetector` inside a `Semantics` wrapper with an `InkWell` wrapped in a `WpFocusRing` and `MergeSemantics` in the history notes section header. Following the established 'house idiom', this enables proper keyboard focus states while retaining correct semantics and hover states via `AnimatedContainer` without interfering visual side effects.
**Action:** When updating interactive list or section headers to support keyboard accessibility, ensure you apply `MergeSemantics` around the label-less `Semantics(button: true)`, and use an `InkWell` + `WpFocusRing` passing a properly disposed `FocusNode`.
