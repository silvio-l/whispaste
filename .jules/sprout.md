...
**Learning:** Adding a button to the NoteEditorPanel toolbar for an action (like Duplicate) that is already supported and plumbed through the widget tree (via  callback) is a low-risk, high-value Sprout task. It exposes existing functionality.
...
**Action:** Always verify if an action already has a callback passed into the widget when considering exposing it in the UI. If it does, implementing it is as simple as adding the UI element.
